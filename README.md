...
```bash
rust-lambda-test$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
rust-lambda-test$ sam local start-api
rust-lambda-test$ curl http://localhost:3000/
```
...
```bash
rust-lambda-test$ sam logs -n HelloWorldFunction --stack-name rust-lambda-test --tail
```
